...
class LaStudio_Metabox {
  ...
  public function render_meta_box_content( $post, $callback ) {
    ...
    global $post, $la_errors, $typenow;

    wp_nonce_field( 'la-studio', 'la-framework-metabox-nonce' );
        ...
        if( $has_nav ) {
          ...
          echo '<div class="la-nav">';
            ...
            foreach( $sections as $value ) {
              ...
              $tab_icon = ( ! empty( $value['icon'] ) ) ? '<i class="la-icon '. $value['icon'] .'"></i>' : '';
  ...
  public function save_post( $post_id, $post ) {
    ...
    if ( wp_verify_nonce( la_get_var( 'la-framework-metabox-nonce' ), 'la-studio' ) ) {
      ...
      foreach ( $this->options as $request_value ) {
        ...
        if( in_array( $post_type, (array) $request_value['post_type'] ) ) {
          ...
          $request = apply_filters( 'la_save_post', $request, $request_key, $post );

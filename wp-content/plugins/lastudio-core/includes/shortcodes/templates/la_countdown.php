...
$css_animation = $count_style = $datetime = $time_zone = $countdown_opts = $tick_col = $tick_size = $tick_line_height = $tick_style = $tick_sep_col = $tick_sep_size = $tick_sep_line_height = '';
...
$countdown_design_style = apply_filters( VC_SHORTCODE_CUSTOM_CSS_FILTER_TAG, vc_shortcode_custom_css_class( $css_countdown, ' ' ), $this->shortcode, $atts );
...
if(!empty($css_animation) && 'none' != $css_animation){
    $el_class .= ' wpb_animate_when_almost_visible la-animation animated';
}
...
$output = '<div class="elm-countdown '.$countdown_design_style.' '.$el_class.' elm-countdown-style-'.$count_style.'" data-animation-class="'.esc_attr($css_animation).'">';

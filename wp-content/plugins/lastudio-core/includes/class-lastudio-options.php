...
class LaStudio_Options{
    ...
    public function validate_save( $request ) {
        ...
        if ( isset( $request['import'] ) && ! empty( $request['import'] ) ) {
            ...
            if( is_array( $decode_string ) ) {
                $add_errors[] = $this->add_settings_error( __( 'Success. Imported backup options.', 'la-studio' ), 'updated' );
        ...
        if ( isset( $request['resetall'] ) ) {
            $add_errors[] = $this->add_settings_error( __( 'Default options restored.', 'la-studio' ), 'updated' );
            set_transient( 'la-framework-transient', array( 'errors' => $add_errors, 'section_id' => $section_id ), $transient_time );
            return;
        }
        ...
        if ( isset( $request['reset'] ) && ! empty( $section_id ) ) {
            ...
            $add_errors[] = $this->add_settings_error( __( 'Default options restored for only this section.', 'la-studio' ), 'updated' );
        }
    ...
    public function admin_page() {
        ...
        if( !$this->settings['disable_header'] ) {
            ...
            echo ($this->settings['ajax_save']) ? '<span id="la-save-ajax">' . __('Settings saved.', 'la-studio') . '</span>' : '';

            submit_button(__('Save', 'la-studio'), 'primary la-save', 'la-fw-save', false, array('data-save' => __('Saving...', 'la-studio')));
            submit_button(__('Restore', 'la-studio'), 'secondary la-restore la-reset-confirm', $this->unique . '[reset]', false);

            if ($this->settings['show_reset_all']) {
                submit_button(__('Reset All Options', 'la-studio'), 'secondary la-restore la-warning-primary la-reset-confirm', $this->unique . '[resetall]', false);
            }

            echo '</fieldset>';
            echo (empty($has_nav)) ? '<a href="#" class="la-expand-all"><i class="fa fa-eye-slash"></i> ' . __('show all options', 'la-studio') . '</a>' : '';
        ...
        foreach ( $this->options as $key => $tab ) {
            ...
            if( ( isset( $tab['sections'] ) ) ) {
                ...
                $tab_icon     = ( ! empty( $tab['icon'] ) ) ? '<i class="la-icon '. $tab['icon'] .'"></i>' : '';
                ...
                foreach ( $tab['sections'] as $tab_section ) {
                    ...
                    $icon = ( ! empty( $tab_section['icon'] ) ) ? '<i class="la-icon '. $tab_section['icon'] .'"></i>' : '';
                ...
                $icon = ( ! empty( $tab['icon'] ) ) ? '<i class="la-icon '. $tab['icon'] .'"></i>' : '';
        ...
        echo '<div class="la-block-left"></div>';
        echo '<div class="la-block-right">'. sprintf('%s <a href="%s">LA-Studio</a>', __('Powered by', 'la-studio' ), 'https://themeforest.net/user/la-studio/portfolio?ref=LA-Studio' ) . '</div>';
        echo '<div class="clear"></div>';

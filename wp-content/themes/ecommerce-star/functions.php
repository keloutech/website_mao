...
define('ECOMMERCE_STAR_THEME_DOC','http://www.ceylonthemes.com/wp-tutorials/');
...
if ( class_exists( 'WP_Customize_Control' ) ) {
	...
	require ECOMMERCE_STAR_TEMPLATE_DIR.'/inc/color-picker/alpha-color-picker.php';
}

/**
 * fontawesome icons functions and filters.
 */
require ECOMMERCE_STAR_TEMPLATE_DIR.'/inc/icon-functions.php';

/**
 * Customizer additions.
 */
 
require ECOMMERCE_STAR_TEMPLATE_DIR.'/inc/customizer.php';

if(!function_exists('ecommerce_star_custom_css')):
	function ecommerce_star_custom_css(){
		require ECOMMERCE_STAR_TEMPLATE_DIR.'/inc/color-patterns.php';
	}
endif;
...
function ecommerce_star_nav_wrap() {
	  ...
	  $wrap .= '<li class="hidden-xs theme-product-cat-menu menu-item menu-item-type-post_type menu-item-object-page menu-item-has-children"><a href="/" class="product-category-heading"><i class="fa fa-align-left"></i>&nbsp;'.esc_html__('All Categories','ecommerce-star').'</a>';
	 ...
	 foreach ($all_categories as $cat) {
		 ...
		 $cat_count++;
			...
			if($cat->category_parent == 0) {
				...
				if($sub_cats) {
				$cat_string .= '<li class="has-sub menu-item menu-item-type-post_type menu-item-object-page menu-item-has-children"> <a href="'.esc_url(get_term_link($cat->slug, 'product_cat')).'">'.esc_html($cat->name).' ('.absint($cat->count).')</a>';
				$cat_string .= '<ul class="sub-menu">';
					foreach($sub_cats as $sub_category) {
						...
						if($sub_sub_cats) {
						$cat_string .= '<li class="has-sub"> <a href="'.esc_url(get_term_link($sub_category->slug, 'product_cat')).'">'.esc_html($sub_category->name).' ('.absint($sub_category->count).')</a>';
							$cat_string .= '<ul class="sub-menu">';
								foreach($sub_sub_cats as $sub_sub_cat) {
									$cat_string .= '<li> <a href="'.esc_url(get_term_link($sub_sub_cat->slug, 'product_cat')).'">'.esc_html($sub_sub_cat->name).' ('.absint($cat->count).')</a>';
								}
							$cat_string .= '</ul>';						
						} else {
						$cat_string .= '<li> <a href="'.esc_url(get_term_link($sub_category->slug, 'product_cat')) .'">'.esc_html($sub_category->name).' ('.absint($cat->count).')</a>';
						}
					}
				$cat_string .= '</ul>'; 
				} else {
					$cat_string .= '<li> <a href="'.esc_url(get_term_link($cat->slug, 'product_cat')).'">'.esc_html($cat->name).' ('.absint($cat->count).')</a>';
				}
			}		      
	 } /* end for each */
	 $cat_string .=  '</ul>';
...
function ecommerce_star_general_admin_notice(){
         ...
         $msg = sprintf('<div data-dismissible="disable-done-notice-forever" class="notice notice-info" >
				...
				esc_html__('Wellcome to eCommerce Star theme','ecommerce-star'),
				esc_url('themes.php?page=ecommerce-star-submenu#getting-started'),			
				esc_html__('Recommended Actions','ecommerce-star'),
				esc_html__('Get most out of eCommerce Start theme, see recommended actions.','ecommerce-star'),	
				esc_html__('Dismiss', 'ecommerce-star'));
		 echo wp_kses_post($msg);
}

if ( isset( $_GET['ecommerce_star_notice_dismissed'] ) ){
	update_option('ecommerce_star_notice', -1);
}

$ecommerce_star_notice = get_option('ecommerce_star_notice', 1);

$ecommerce_star_refreshed = isset($_SERVER['HTTP_CACHE_CONTROL']) && ($_SERVER['HTTP_CACHE_CONTROL'] === 'max-age=0' ||  $_SERVER['HTTP_CACHE_CONTROL'] == 'no-cache');

if (($ecommerce_star_notice > 0 || $ecommerce_star_notice == '') && !$ecommerce_star_refreshed ){

...
class Optima_Setting{
    ...
    public function __construct() {
        $this->args = array(
            'option_name'       => Optima::get_option_name(),
            'post_meta_name'    => Optima::get_original_option_name(),
            'term_meta_name'    => Optima::get_original_option_name()
        );
    }
    ...
    public function get( $key = '', $default = '' ){
        ...
        return apply_filters("optima/setting/option/get_{$key}", $value);
    }
        ...
        if(!empty($sub_key)){
            if( $single ) {
                if(isset($object_value[$sub_key])){
                    return apply_filters("optima/setting/post_metadata/get_{$sub_key}", $object_value[$sub_key]);
                }
                else{
                    return apply_filters("optima/setting/post_metadata/get_{$sub_key}", false);
                }
            }
            else{
                $tmp = array();
                ...
                return apply_filters("optima/setting/post_metadata/get_{$sub_key}", $tmp);
            }
        }
        else{
            return apply_filters("optima/setting/post_metadata/get_{$meta_key}", $object_value);
        }
    }
    ...
    public function get_term_meta( $object_id, $sub_key = '', $meta_key = '', $single = true ) {
        ...
        if(!empty($sub_key)){
            if( $single ) {
                if(isset($object_value[$sub_key])){
                    return apply_filters("optima/setting/term_metadata/get_{$sub_key}", $object_value[$sub_key]);
                }
                else{
                    return apply_filters("optima/setting/term_metadata/get_{$sub_key}", false);
                }
            }
            else{
                $tmp = array();
                ...
                return apply_filters("optima/setting/term_metadata/get_{$sub_key}", $tmp);
            }
        }
        else{
            return apply_filters("optima/setting/term_metadata/get_{$meta_key}", $object_value);
        }
    }
    ...
    public function get_setting_by_context( $key, $default = '', $context = array()){
        ...
        elseif(in_array('is_archive', $context)){
            ...
            if(in_array('is_shop', $context)){
                ...
                if(Optima()->get_page_id()){
                    $_value = $this->get_post_meta( Optima()->get_page_id(), $key);
                    if(!empty($_value) && $_value !== 'inherit'){
                        $value = $_value;
                    }
                }
        ...
        return apply_filters('optima/setting/get_setting_by_context', $value, $key, $context);
    }

...
class Optima_Breadcrumbs {
	 ...
	 */
	public $css = array(
		'module'    => 'la-breadcrumbs',
		'content'   => 'la-breadcrumb-content',
		'wrap'      => 'la-breadcrumb-wrap',
		'browse'    => 'la-breadcrumb-browse',
		'item'      => 'la-breadcrumb-item',
		'separator' => 'la-breadcrumb-item-sep',
		'link'      => 'la-breadcrumb-item-link',
		'target'    => 'la-breadcrumb-item-target',
	);
	...
	public function __construct( $args = array() ) {
		...
		$defaults = array(
			...
			'show_mobile'       => false,
			'show_tablet'       => true,
			'wrapper_format'    => '<div>%2$s</div>',
			'page_title_format' => '<header><div class="page-title h4">%s</div></header>',
			...
			'labels'            => array(),
			'date_labels'       => array(),
			'action'            => 'optima/action/breadcrumbs/render_html',
			'css_namespace'     => array(),
			'path_type'         => 'full', // minified , full
			'post_taxonomy'     => apply_filters(
				'optima/filter/breadcrumbs/trail_taxonomies',
				array(
					'post'      => 'category',
				)
			),
		...
		$this->args = apply_filters( 'optima/filter/breadcrumbs/args', wp_parse_args( $args, $defaults ) );
	...
	public function get_trail() {
		...
		$wrapper_classes = apply_filters( 'optima/filter/breadcrumbs/wrapper_classes', $wrapper_classes );
	...
	public function get_items() {
		...
		$breadcrumb = apply_filters( 'optima/filter/breadcrumbs/trail', $breadcrumb, $this->args );
	...
	public function default_labels() {
		...
		if ( $use_custom_front_title ) {
			$default = esc_html__( 'Home', 'optima' );
			$page_on_front_title = $default;
			$page_on_front_title = $this->prepare_label( $page_on_front_title, $default );
		}

		$labels = array(
			'browse'              => esc_html__( 'Browse:', 'optima' ),
			'home'                => $page_on_front_title,
			'error_404'           => esc_html__( '404 Not Found', 'optima' ),
			'archives'            => esc_html__( 'Archives', 'optima' ),
			'search'              => esc_html__( 'Search results', 'optima' ),
			'paged'               => esc_html__( 'Page %s', 'optima' ),
			'archive_minute'      => esc_html__( 'Minute %s', 'optima' ),
			'archive_week'        => esc_html__( 'Week %s', 'optima' ),
	...
	public function build_trail() {
		...
		$custom_trail = apply_filters( 'optima/filter/breadcrumbs/custom_trail', false, $this->args );
		 ...
		 * @var   array
		 */
		$this->items = apply_filters( 'optima/filter/breadcrumbs/items', $this->items, $this->args );
...
//			$this->_add_item( 'link_format', $label, $url );
	...
	public function prepare_label( $label, $fallback = false ) {
		...
		if ( false !== $fallback ) {
			$fallback = sprintf(
				apply_filters( 'optima/filter/breadcrumbs/icon_fallback', '<span class="hidden">%s</span>', $fallback ),
				esc_attr( $fallback )
			);
		}
		...
		// Return default icon
		return sprintf(
			apply_filters( 'optima/filter/breadcrumbs/default_icon_format', '<span class="%1$s">%2$s</span>', $label, $fallback ),

...
class Optima {
    ...
    private static $original_option_name = 'optima_options';
    ...
    private function __construct() {
        ...
        wp_cache_add_non_persistent_groups( 'optima' );
        ...
        $this->settings       = Optima_Setting::get_instance();
        $this->init           = new Optima_Init();
        $this->template       = new Optima_Template();
        $this->blog           = new Optima_Blog();
        $this->images         = new Optima_Images();
        $this->layout         = new Optima_Layout();

        add_action( 'wp', array( $this, 'set_page_id' ) );
        add_action( 'wp', array( $this, 'set_current_context' ) );
        add_action( 'wp', array( $this, 'build_crumbs'));

        new Optima_Widget_CssClass();

        do_action( 'optima_core_loaded' );
    }
    ...
    public function build_crumbs(){
        ...
        $this->breadcrumbs = new Optima_Breadcrumbs( $args );
    }

    public function set_is_updating() {
        if ( ! self::$is_updating && $_GET && isset( $_GET['optima_update'] ) && '1' == $_GET['optima_update'] ) {
    ...
    public static function multilingual_options() {
        // Set the self::$lang.
        if ( ! in_array( Optima_Multilingual::get_active_language(), array( '', 'en', 'all' ) ) ) {
            self::$lang = '_' . Optima_Multilingual::get_active_language();
        }

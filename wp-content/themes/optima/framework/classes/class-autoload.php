...
/**
 * The Autoloader class for Optima.
 */
class Optima_Autoload {
    ...
    public function __construct( $args = array() ) {
        ...
        if ( empty( self::$transient_name ) ) {
            self::$transient_name = 'optima_autoloader_paths_' . md5( __FILE__ );
        }
    ...
    protected function get_path( $class_name ) {
        ...
        $paths = array();
        if ( 0 === stripos( $class_name, 'Optima' ) ) {
            $path     = wp_normalize_path( Optima::$template_dir_path . '/framework/classes/' );

            $class_name = str_replace( array( 'Optima_' ), '', $class_name );
